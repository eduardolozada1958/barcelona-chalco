/** Temporada por defecto si Ajustes del club no define otra. */
export const CURRENT_SEASON = '2026';

/** Logo en rutas públicas (WebP, carpeta public/images). */
export const CLUB_LOGO_URL = '/images/logo.webp';
/** Nombre visible del sitio / proyecto. */
export const CLUB_DISPLAY_NAME = 'F.C. Barcelona Cupido';
/** Mismo nombre en textos legales (plataforma web, no federación ni club profesional). */
export const CLUB_LEGAL_NAME = 'F.C. Barcelona Cupido';
/** Sitio público (sin barra final). Preferir VITE_PUBLIC_APP_URL en producción. */
export const CLUB_SITE_URL = (
  import.meta.env.VITE_PUBLIC_APP_URL || 'https://barcelona-chalco.pages.dev'
).replace(/\/$/, '');

/** Aclaración breve para páginas legales. */
export const SITE_LEGAL_DISCLAIMER =
  'Este sitio es una plataforma web para consultar estadísticas, partidos, avisos e identidad de jugadores. No constituye por sí misma un club deportivo federado ni una entidad distinta a la que administra el contenido.';
