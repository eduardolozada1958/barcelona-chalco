...
export class QrService {
   ...
   */
  static async generateImage(playerId: string, qrToken: string, width = 384): Promise<Buffer> {
    const token = qrToken.trim();
    if (!token) {
      throw new BadRequestError('Token QR requerido');
    }

    const pngWidth = Math.min(512, Math.max(192, width));
    ...
    const memKey = `qr:png:${player.id}:${player.qr_token}:${pngWidth}`;
